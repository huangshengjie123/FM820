...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(getDataShow:) name:@"dataNoti" object:nil];
...
- (void)getDataShow:(NSNotification *)noti{
    
    NSDictionary *dataDic = noti.userInfo;
    dataArray = [dataDic objectForKey:@"dataArr"];
...
- (void)drawAxisLine{
    ...
    self.xAxisLine.backgroundColor = [UIColor whiteColor];
...
#pragma mark - 设置x轴标题下方Label


- (void)setXLineNameLabelBottom:(NSArray *)dataArr{
    if (dataArr.count > 0) {
        ...
        for (NSInteger i = 0; i < dataArr.count; i++) {
            ...
            CGFloat height = self.frame.size.height - self.xAxisLine.xLineStartYPos - _xLineNameLabelToXAxisLinePadding;
            CGFloat center_xPos = self.xAxisLine.xLineStartXPos + _groupPadding + (_groupWidth + _groupPadding) * i + width * 0.5;
            CGFloat center_yPos = self.yAxisLine.yLineStartYPos + _xLineNameLabelToXAxisLinePadding + height * 0.5+20;
            
            //label的中心点
            CGPoint label_center = CGPointMake(center_xPos, center_yPos);
            CGRect rect = [dataArr[i] stringWidthRectWithSize:CGSizeMake(width + _groupPadding * 0.5, height) font:_xLineNameFont];
            ZFLabel * label = [[ZFLabel alloc] initWithFrame:CGRectMake(0, 0, rect.size.width, rect.size.height)];
...
- (void)strokePath{
    ...
    [self setXLineNameLabelBottom:dataArray];
